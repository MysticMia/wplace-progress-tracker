...
def _save_pixel_count_data(
        config: Config,
        current_data: dict[ColorName, int],
    ...
    with (open(path, "w") as f):
        max_key_length = max(len(key) for key in current_data.keys())
        max_progress_count_length = len(str(
            max(val for val in current_data.values())))
        max_template_count_length = len(str(
            max(val for val in template_data.values())))

        f.write(
            f"{'Color':^{max_key_length}}   "
            f"{'Remaining':<{max_progress_count_length}}   "
        ...
        for key, remaining_count in current_data.items():
            ...
            f.write(
                ...
                f"{remaining_count:>{max_progress_count_length}}"
                f"         "
...
def save_pixel_count(config: Config):
    ...
    placed_pixel_count = get_pixel_count(img)
    goal_pixel_count = get_pixel_count(template)
    _save_pixel_count_data(config, placed_pixel_count, goal_pixel_count)
